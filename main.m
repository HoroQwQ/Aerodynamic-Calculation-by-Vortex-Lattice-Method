...
%% Calculate the reference area, etc
if type == 1
...
hold on
if type == 0
    scatter(0,3,0.5,Col_2);
    hold on
elseif type == 2
    scatter(0,1.2,0.5,Col_2);
    hold on
end
...
h3=scatter(z2,x2,8,Col_3,'.');
 ...
 zz=z1;     z1=z2;     z2=zz;
%LEFT
h4=scatter(-z0,x0,5,'b','+');
hold on
h5=scatter(-z1,x1,8,Col_3,'.');
hold on
h6=scatter(-z2,x2,8,Col_3,'.');
legend([h0(1),h1(1),h2(1),h3(1)],'Mesh','Pneumatic center', 'Vortex origin', 'Vortex Outlet','location', 'northeast')
title('Swept wing meshing');
xlabel('Z');    ylabel('X');
% Merge the left and right wings
 z0=[-z0 z0];x0=[x0 x0];
 z1=[-z1 z1];x1=[x1 x1];
 z2=[-z2 z2];x2=[x2 x2];
%% Solve
for j=1:3
    for i=-4:10
        alpha=i.*Rad;
        [varGamma,K,F]= airDynamic_CalVortex(x0,x1,x2,z0,z1,z2,Vinf(j),alpha,2*(sqrt(n)-1)^2);
        Cy(j,i+5)=airDynamic_CalLiftingForce(Vinf(j),RHO,dZ,S,varGamma/2,2*(sqrt(n)-1)^2);
    end
end
disp(Cy);
